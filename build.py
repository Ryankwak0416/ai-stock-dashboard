...
function renderMindmap(){
  ...
  var palette={"AI 반도체-HBM/메모리":"#f87171","AI 서버 PCB/기판":"#fb923c","AI 후공정/장비":"#fbbf24","AI 팹리스":"#a78bfa","AI 소프트웨어-대형":"#34d399","AI 소프트웨어-소형":"#2dd4bf","AI 의료영상":"#22d3ee","AI 로봇":"#60a5fa","AI 인프라/클라우드":"#818cf8","AI 보안":"#f472b6","AI 콘텐츠/VFX":"#c084fc"};
  var cats=Object.keys(palette);
  var nodes=STOCKS.map(function(st){return {data:{id:st.code,label:st.name,cat:st.category,color:palette[st.category]||"#8b93a1",size:(st.capNum>0?Math.max(36,Math.min(96,Math.log10(st.capNum)*9-58)):36)}};});
  [["nvidia","NVIDIA"],["tsmc","TSMC"],["spacex","스페이스X"]].forEach(function(e){nodes.push({data:{id:e[0],label:e[1],cat:"외부 핵심축",color:"#e5e7eb",size:46,ext:1}});});
  ...
  var cy=cytoscape({container:document.getElementById("mmcy"),elements:{nodes:nodes,edges:edges},style:[
    ...
    {selector:"node[ext]",style:{"shape":"round-diamond","color":"#0b0e13","border-color":"#94a3b8"}},
    {selector:"edge",style:{"width":1.4,"line-color":"#39435a","curve-style":"bezier","opacity":0.5,"transition-property":"opacity line-color width","transition-duration":"0.15s"}},
  ...
  cy.on("tap","node",function(ev){var n=ev.target;cy.scratch("_pin",n.id());focusOn(n);document.getElementById("mminfo").innerHTML="<b style='color:#e7e9ee'>"+n.data("label")+"</b> · "+n.data("cat")+" · 직접 연결 "+n.connectedEdges().length+"건";});
  cy.on("tap","edge",function(ev){var d=ev.target.data();cy.scratch("_pin",d.id);cy.batch(function(){cy.elements().addClass("dim").removeClass("hl");ev.target.removeClass("dim").addClass("hl");ev.target.connectedNodes().removeClass("dim").addClass("hl");});document.getElementById("mminfo").innerHTML="<b style='color:#6ea8ff'>"+cy.getElementById(d.source).data("label")+" ↔ "+cy.getElementById(d.target).data("label")+"</b> — "+d.label;});
