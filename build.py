...
function renderMindmap(){if(typeof cytoscape==="undefined")return;var catColors={"AI 반도체-HBM/메모리":"#ff6b6b","AI 서버 PCB/기판":"#f59e0b","AI 후공정/장비":"#fbbf24","AI 팹리스":"#a78bfa","AI 소프트웨어-대형":"#34d399","AI 소프트웨어-소형":"#6ee7b7","AI 의료영상":"#22d3ee","AI 로봇":"#60a5fa","AI 인프라/클라우드":"#818cf8","AI 보안":"#f472b6","AI 콘텐츠/VFX":"#c084fc"};var nodes=STOCKS.map(function(s){return {data:{id:s.code,label:s.name,cat:s.category,color:catColors[s.category]||"#8b93a1",size:(s.capNum>0?Math.max(26,Math.min(66,Math.log10(s.capNum)*6-28)):26)}};});[["nvidia","NVIDIA"],["tsmc","TSMC"],["spacex","스페이스X"]].forEach(function(e){nodes.push({data:{id:e[0],label:e[1],cat:"외부 핵심축",color:"#9ca3af",size:34,ext:1}});});var E=[["005930","000660","HBM 메모리 양대 축"],["042700","000660","TC본더 HBM 핵심장비 납품"],["042700","005930","HBM 장비 공급"],["039030","000660","반도체 레이저장비"],["095340","000660","테스트 소켓"],["403870","000660","고압어닐링 HBM 공정"],["007660","nvidia","엔비디아향 MLB 기판"],["042700","spacex","스페이스X 지분 취득"],["403870","042700","스페이스X 테마 동반"],["399720","054450","디자인 파트너"],["399720","005930","삼성 파운드리 디자인하우스"],["445090","tsmc","TSMC 디자인 파트너"],["277810","005930","삼성전자 지분투자"],["454910","277810","협동·휴머노이드 로봇"],["090360","277810","로봇 테마 동반"],["056080","277810","자율주행 로봇 테마"],["338220","328130","AI 의료영상"],["338220","322510","의료AI 동반 급등"],["108860","338220","의료AI 동반"],["328130","322510","AI 의료영상"],["035420","035720","대형 플랫폼 AI"],["299900","289220","VFX·버추얼휴먼"]];var edges=E.map(function(e){return {data:{source:e[0],target:e[1],label:e[2]}};});var card=document.createElement("div");card.className="card";card.innerHTML="<h3 style='margin:0 0 8px;font-size:14px'>관계 마인드맵 · 업체 연결도</h3><div class='note' style='margin-bottom:8px'>색 = 카테고리, 크기 = 시가총액. 선 = 공급·지분·파트너·테마 관계(코멘트·뉴스 기반 큐레이션). 노드 드래그·휠 줌, 선 클릭 시 근거 표시.</div><div id='mmcy' style='height:560px;border:1px solid #232a35;border-radius:10px;background:#0d1117'></div><div id='mminfo' class='note' style='margin-top:6px'>선 또는 노드를 클릭하세요.</div>";var ref=document.getElementById("headlines").closest(".card");ref.parentNode.insertBefore(card,ref);var cy=cytoscape({container:document.getElementById("mmcy"),elements:{nodes:nodes,edges:edges},style:[{selector:"node",style:{"background-color":"data(color)","label":"data(label)","color":"#e7e9ee","font-size":"11px","font-weight":"600","text-valign":"center","text-halign":"center","width":"data(size)","height":"data(size)","text-outline-color":"#0d1117","text-outline-width":2}},{selector:"node[ext]",style:{"shape":"round-rectangle","color":"#0d1117","font-weight":"700"}},{selector:"edge",style:{"width":1.5,"line-color":"#3a4453","curve-style":"bezier","opacity":0.6}},{selector:"edge:selected",style:{"line-color":"#6ea8ff","width":3.5,"opacity":1}},{selector:"node:selected",style:{"border-width":3,"border-color":"#6ea8ff"}}],layout:{name:"cose",animate:false,padding:24,nodeRepulsion:9000,idealEdgeLength:95,nodeOverlap:14},minZoom:0.3,maxZoom:2.5,wheelSensitivity:0.2});cy.on("tap","edge",function(ev){var d=ev.target.data();document.getElementById("mminfo").innerHTML="<b style='color:#6ea8ff'>"+cy.getElementById(d.source).data("label")+" ↔ "+cy.getElementById(d.target).data("label")+"</b> — "+d.label;});cy.on("tap","node",function(ev){var n=ev.target;document.getElementById("mminfo").innerHTML="<b style='color:#e7e9ee'>"+n.data("label")+"</b> · "+n.data("cat");});}function init(){
