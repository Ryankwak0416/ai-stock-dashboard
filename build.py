...
function renderMindmap(){
  ...
  var nodes=STOCKS.filter(function(st){return ids[st.code];}).map(function(st){var col=(st.market==="코스피")?KP:KD;var sz=(st.capNum>0?Math.max(54,Math.min(116,Math.log10(st.capNum)*11-64)):54);return {data:{id:st.code,label:st.name,cat:st.category,color:col,size:sz}};});
  [["nvidia","NVIDIA"],["tsmc","TSMC"],["spacex","스페이스X"]].forEach(function(e){nodes.push({data:{id:e[0],label:e[1],cat:"외부 핵심축",color:EX,size:66,ext:1}});});
